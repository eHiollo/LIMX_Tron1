...
ISAACLAB_VERSION="2"
while true; do
    read -p "Please enter the IsaacLab Version (1 for v1.4.1, 2 for latest v2.3.x): " ISAACLAB_VERSION

    if [ "$ISAACLAB_VERSION" == "1" ]; then
        echo -e "${GREEN}Installing IsaacLab v1.4.1...${RESET}"
        break
    elif [ "$ISAACLAB_VERSION" == "2" ]; then
        echo -e "${GREEN}Installing latest IsaacLab v2.3.x...${RESET}"
        break
    else
        echo -e "${RED}Invalid input. Please enter 1 for v1.4.1 or 2 for the latest v2.3.x.${RESET}"
    fi
done
...
if [ "$ISAACLAB_VERSION" == "1" ]; then
    ...
    echo -e "${GREEN}Installing torch==2.7.0 and torchvision==0.22.0 for latest IsaacLab v2.3.x...${RESET}"
...
if [ "$ISAACLAB_VERSION" == "1" ]; then
    ...
    echo -e "${GREEN}Installing latest IsaacSim v5.1.0 (v2.3.x)...${RESET}"
